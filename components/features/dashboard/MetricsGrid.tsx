    ...
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 mb-6">
      {loading
        ? skeletons.map((_, idx) => (
            <div
              key={idx}
              className="flex flex-col justify-between p-4 rounded-lg bg-slate-100 animate-pulse h-28"
            >
              <div className="h-4 w-2/3 bg-slate-200 rounded mb-2" /> {/* Title */}
              <div className="h-6 w-full bg-slate-200 rounded mb-2" /> {/* Value */}
              <div className="h-3 w-1/3 bg-slate-200 rounded" /> {/* Change / trend */}
            </div>
          ))
        : (
          <>
