...
import { User, Scale, Gem, DollarSign, Calendar, Hash, Tag, Barcode } from "lucide-react";
      ...
      <Table>
        ...
        <TableHeader className="bg-slate-50 dark:bg-gray-900">
          <TableRow>
            ...
            <TableHead><div className="flex items-center gap-2"><Hash className="h-3.5 w-3.5" />Token ID</div></TableHead>
            <TableHead><div className="flex items-center gap-2"><DollarSign className="h-3.5 w-3.5" />Value</div></TableHead>
        ...
        <TableBody>
          {assets.map((asset) => (
            <TableRow key={asset.token_id} className="hover:bg-slate-50 dark:hover:bg-gray-800/50 transition-colors">
              ...
              <TableCell>
                <Link href={`/assets/${asset.token_id}`}>
