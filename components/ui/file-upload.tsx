...
export const FileUpload: React.FC<FileUploadProps> = ({
  ...
  const handleFileChange = (file: File | null) => {
    if (disabled) return; // ✅ prevent action
  ...
  return (
    <div className={`${className} ${disabled ? "opacity-60" : ""}`}>
      ...
      <div
        className={`relative ${
          !value && !disabled ? "cursor-pointer" : "cursor-not-allowed"
        }`}
        onClick={
          !value && !disabled ? () => inputRef.current?.click() : undefined
        }
        onDrop={handleDrop}
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-0.5">
                  ...
                  <Button
                    ...
                    variant="ghost"
                    size="sm"
                    disabled={disabled} // ✅ disable remove
                    onClick={(e) => {
                      e.stopPropagation();
